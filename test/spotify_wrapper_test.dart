...
void main() {
  group('Enum Utils', () {
    ...
    test('SpotifyType from Int', () {
      final possibleEnum = EnumUtils.enumFromInt<SpotifyType>(2);
      expect(possibleEnum, SpotifyType.Track);
    });

    test('Enum from String with Error: value not found', () {
      final possibleEnum = EnumUtils.enumFromString<SpotifyType>("Exception");
      expect(possibleEnum, SpotifyType.Error);
    });

    test('Enum from Int with Error: out of bounds', () {
      final possibleEnum = EnumUtils.enumFromInt<SpotifyType>(99);
      expect(possibleEnum, SpotifyType.Error);
    });
  });
